...
{
    if (!relativePath) { return Bundle.Base.bundlePath; }
    NSString *base = [Bundle bundleWith: relativePath].bundlePath;
    return base;
}

NSString *MPPathToDataFile(NSString *name, NSString *dirPath) {
    return [NSString pathWithComponents:@[MPDataDirectory(dirPath), name]];
}
...
NSURL *MPHighlightingThemeURLForName(NSString *name)
{
    ...
    if ([name hasExtension:@"css"]) { name = name.stringByDeletingPathExtension; }

    NSURL *url = [[Bundle Prism] URLForResource:name withExtension:@"css"
                           subdirectory:@"themes"];

    // Safty net: file not found, use default.
    if (!url)
    {
        url = [Bundle.Prism URLForResource:@"prism" withExtension:@"css"
                        subdirectory:@"themes"];
    }
    return url;
}
